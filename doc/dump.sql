...
CREATE TABLE IF NOT EXISTS `ipn_orders` (
  ...
  `order_status` enum('PAID','WAITING','REJECTED') DEFAULT NULL COMMENT
'(DC2Type:enumorderstatus)',
...
CREATE TABLE IF EXISTS `project_idea` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
...
--
ALTER TABLE `user`
  ADD CONSTRAINT `fk_user_event` FOREIGN KEY (`event_id`) REFERENCES
`event` (`event_id`) ON DELETE NO ACTION ON UPDATE NO ACTION;
